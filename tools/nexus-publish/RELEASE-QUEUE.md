...
### 2026-06-29 (cycle 9)
- **Mass Deconstruct (MassDeconstruct) -> 1.4.0-beta** (mod 58) — PUSHED. Live footprint highlight: bold red fill over each building in the drag box (previously selection was invisible). Drag now uses native game-camera world-point — fixes offset on elevated/real-world-tile maps. Also includes 1.3.0 pacing (<=50 concurrent deconstructs, prevents navmesh crash on mass-clear). Requires API 1.2.1+.
