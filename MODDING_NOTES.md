...
- ⚠️ **Scavenge vs expedition loot are TWO branches in one method.** `ResourcesToFindProvider`
  `.DrawResourcesForTag(poi, cubicMeter, amountMultiplier, expedition)`:
  scavenge → `resourceAmount = Round(Random(min,max) * amountMultiplier)` (multiplier honoured);
  expedition → `resourceAmount = (int)(min + max/2)` **ignores `amountMultiplier` entirely**.
  So a prefix scaling `amountMultiplier` lifts buildings only. `min`/`max` are ∝ `cubicMeter` (+ un-scaled
  `Min/MaxLineFunctionShift` constants), so scale `cubicMeter` when `expedition==true` to lift expedition
  loot (≈linear). Both callers feed through `DrawResourceForGameplayGroup`. (Surrounded 0.2.2, 2026-06-17,
  reported by Furippu)
- 🧭 **Lair FORMATION pipeline (swarm → permanent lair):** `SwarmGroupsToLairConverter` (`IUpdatable`)
  watches `GroupEnteredEnterableSignal` — when swarm groups hole up in a building they become
  conversion *candidates*; after a delay the building converts to a hideout/lair. Gated entirely by
  `HideoutsConfig` getters (top-level class, no namespace): `MinGroupsAmountToLairConvert` (int, groups
  needed), `ConvertDelayGts` (float, gameplay-time delay), `UseChanceToConvert`/`ChanceToConvert`,
  `MinCharactersAmountToConvert`, `MinBuildingVolumeToConvertLair`. **This is vanilla, NOT driven by
  Surrounded** — but the getters are clean Harmony postfix targets: divide `MinGroups` (floor 1) +
  `ConvertDelayGts` by a factor to make lairs form more/less readily. Surrounded 0.4.0 reads them
  read-only for an F1 "Status" read-out and scales them only via an opt-in lever. (Surrounded 0.4.0,
  2026-06-18, requested by Meefstick)
- 🧭 **F1 read-out of effective values (ConfigurationManager):** to *show* (not edit) live computed
  numbers, bind read-only `ConfigEntry<string>` entries and update `.Value` on a slow `Update()` tick;
  mark them read-only with a locally-defined `ConfigurationManagerAttributes { ReadOnly=true,
  HideDefaultButton=true }` (CM matches the class by NAME via reflection — no assembly ref needed; extra
  fields are harmless, just trigger CS0649). Capture the real before/after inside the existing getter
  postfixes into statics rather than re-deriving — shows exactly what the game asked vs what the mod
  returned. (Surrounded 0.4.0, 2026-06-18)
...
| Surrounded | Siege mode: scales `SwarmsIntensity`+`HordeSizeMultiplier` getters (native swarms, more/bigger) + scavenge **& expedition** loot ×N (v0.2.2: expedition branch ignores `amountMultiplier`, scale `cubicMeter` instead). NOT hand-spawned (that crashes). v0.4.0: F1 "Status" read-out (live effective freq/horde/lair numbers) + opt-in lair-formation lever (`HideoutsConfig` getters). |
...
| LowCameraAngle | Reflection-pokes `GameCameraParameters` clamps for flatter/lower cam. No Harmony. See Camera rig below. |
| SquadGrenades | Idle player squads auto-throw grenades / fire mortars at in-reach hostile clusters via `AttackOnGroundOrder`. Reach-gated (no advance) + friendly-blast guard. See Ordnance below. |

## Camera rig (`Cameras.GameCamera : TmsCamera`)

- 🧭 Params live in `Cameras.GameCameraParameters` (a Sirenix `Config`/ScriptableObject). Base
  `TmsCamera`/`TileMapSystemCamera` (NOT decompiled — separate lib) reads everything through
  interface getters **every frame**, so poking the params asset = live effect, **no Harmony**.
- 🧭 Reach asset via `GameCamera.Active` (static) → private field `_gameCameraParameters`.
- 🧭 Clamp fields (private serialized): `_minXAxisAngle`/`_maxXAxisAngle` (pitch tilt degs;
  `_minXAxisAngle` = "can't go flatter" wall), `_xAxisAngleCurve` (zoom→tilt coupling),
  `_minYDistance`/`_maxYDistance` (zoom height), `_pitchWithMouse` (bool), `_unlockVerticalAxis`
  (true → `MaxZoom` returns 10000).
- 🧭 Free-tilt already exists: `PitchWithMouse` getter = `_pitchWithMouse && !CameraFixedTilt`.
  "Camera Fixed Tilt" toggle in Escape→Settings (default OFF). You're just clamped by min-angle.
- ⚠️ Low + flat clips terrain → bump `UnityCamera.nearClipPlane`. Units are billboards
  (`BillboardScaling`) — flatten at low angle, expected.

## Ordnance / AttackOnGroundOrder (grenades + mortars)

- 🧭 **Grenade throw = an order, not custom code.** Manual flow (panel `ThrowGrenadeInfoPanelButton`
  → `SquadWeaponCursor` `AccessoryGrenadeManualTargeting` → click) just issues:
  `group.OrdersHandlers.GiveOrder(new AttackOnGroundOrder(new AttackOnGroundInfo(point, accessory)), …)`.
  Order handles projectile, cooldown, resource drain, `Use()`. Auto-use = issue the same order.
- 🧭 Grenade = **accessory** (`GrenadesAccessoryResource : AttackableAccessoryResource`, `res_grenades`).
  `Group.AccessoryHandler`: `IsGrenadesAccessoryActive`, `HasGrenadesAccessory`,
  `TryGetAccessoryResource(ResourceID, out AccessoryResource)`. Weapon = `acc.WeaponAccessoryResourceData.Weapon`.
  Per-char cooldown: `acc.HasCooldown(user)`, `acc.GetAvailableUser()`.
- 🧭 Mortar = **weapon** (`eq_mortar`, no accessory class). `AttackOnGroundOrder` counts attackers by
  `weapon.CanAttackOnGround`; issue plain `new AttackOnGroundOrder(point)` → all ground-attack weapons
  in squad fire. `Weapon.IsExplosive`, `Weapon.ExplosionSettings` (struct, `.radius`).
- ⚠️ **SAFETY CRUX:** `AttackOnGroundOrder.StartOrder()` — if target distance > weapon reach, it
  **moves the squad toward the target** until in range. So auto-fire MUST gate on
  `distance <= reach` (`weapon.Stats.GetReach(group.IsOnExpedition(), char.GetSkillReachModificationValue())`)
  or squads charge the horde. In-range → squad stays planted.
- ⚠️ **PLAYER SQUADS ≠ `GroupsController`.** `GroupsController._groups` holds **infected** groups only
  (its `Fraction.Player` filter matches ~nothing — verified `players=0` always). Player squads live in
  **`SquadsController.Squads`** (`IReadOnlyList<Group>`). Get the instance from IFZModAPI `Cache.Squads`
  (cached via `[HarmonyPatch(typeof(SquadsController),"Inject")]` Postfix). This was the bug that made
  SquadGrenades silently do nothing for several builds. (SquadAutoBehavior's same Player-filter on
  GroupsController is almost certainly dead too.)
- ⚠️ **Enemy detection: use `group.EnemiesProvider.GetAllEnemiesInFightRange(null)`** (null filter =
  ALL enemies in fight range). The `StancesBehaviourHelper.GetAllNotAttackedEnemiesInRange()` variant
  passes `_canStopToAttackFunc` which **excludes already-engaged/melee enemies** — so a squad fighting a
  swarm saw only distant unattacked hordes (nearest=108m vs reach=54 → never threw). Range used is the
  squad's **FightRange** (vision, ~100m+), so gate to weapon reach yourself.
- 🧭 Tick driver: Harmony `GroupsController.CustomUpdate` Postfix is a fine per-frame heartbeat (fires
  reliably even though it holds infected) — but iterate `Cache.Squads.Squads` for the player roster, not
  `__instance`. Filter `Fraction.Player` + `IsInState(StateType.Idle)` (only 5 StateTypes exist;
  in-building & in-combat squads are still `Idle` — combat is handled *inside* IdleState).
- 🧭 Game scale: grenade `GetReach` ≈ 54 units; enemy distances ~100m+ in fight range. Tune blast/cluster
  radii in tens-of-units. **STATUS: working after the Cache.Squads + EnemiesProvider fixes** (user
  confirmed "seemed to work"). DebugLog config still ON — turn off once tuned.
- ⚠️ Build needs `Sirenix.Serialization.dll` ref (CombatStats/`Weapon.Stats` is a SerializedScriptableObject).
  `ICharacter` is in `Gameplay.Units.Virtual` (not `.Characters`).
- 🧭 Console give: open with backtick `` ` ``, `AddResourcesToHq res_ammo 1000` (`[GameCommand]`,
  `GameConsoleCommandHandler.cs:1265`). `EnableCheats` unlocks locked cmds. `AddCommonResources` = wood/metal/bricks.

## Combat damage / accuracy (planned: move-fire + confidence mod)

- 🧭 **No hit/miss roll for ranged.** Damage is deterministic distance-falloff via
  `CombatStats.GetDamageForDistance(dist, expedition, sharpshooter, reachMod)`. `GetChanceToHit` exists
  but is **referenced only in its own file** = UI/AI flavor, NOT real combat. So "accuracy penalty" must
  be a **damage multiplier**, not a miss chance.
- 🧭 **Damage seam = `CharacterFightHandler.GetDamage(weapon, opponent)`** (private; `_character` gives
  full context: `.Movement.IsMoving()`, `.IsInsideEnterable()`, `.VehicleModule.InVehicle`,
  `.Group.EnemiesProvider`). One Harmony Postfix scales returned dmg → confidence/movement model.
- 🧭 **Move-and-fire** = AI stop-to-shoot pause: `IdleState.PauseOrdersExecution` + `_stopToShootCooldown`,
  decided by `StancesBehaviourHelper.CanStopWhileMoveToOrder`/`CanStopToAttackEnemy`, tuned by
  `GroupDraft.StopToShoot{CooldownGTS,MoveToOrderBreakRange01,MoveToOrderBreakDistance}`. Suppress pause to
  keep moving while firing; verify shots still issue mid-move (driving+fire mod = precedent). Movement
  dmg-penalty only meaningful once this lands; confidence (swarm-panic + indoor-steady) works standalone now.

## Fog-of-war reveal + Flares mod (0.1.2, 2026-06-18)

- 🧭 **Reveal FoW from a mod WITHOUT instantiating a `FogOfWarObserver`.** The observer's per-frame work
  (`FogOfWarObserver.CustomUpdate`) is just: `_fogOfWarObserversCountFinder.Add(new Observer{Position,Radius})`
  + `_fogOfWarUnitsVoid.Add(position, radius)` — both **Zenject singletons, both `Add` public**. Pull the two
  instances once by reflecting a live squad's `Character._fogOfWarObserver` (private) → its `_fogOfWarObserversCountFinder`
  / `_fogOfWarUnitsVoid` fields. Then push `(pos, radius)` into them every frame. No GameObject/collision-detector/inject needed.
- 🧭 **Bunker/tower mounted weapons ≠ squads.** A bunker mortar is a `StructureDefenceModule` (MonoBehaviour;
  `Object.FindObjectsOfType<StructureDefenceModule>()`), with `.SelectedWeapon` (the mounted weapon),
  `.transform.position`, `.LastAttackedGroup/Character`. Mortar = `SelectedWeapon.CanAttackOnGround && IsExplosive`.
  Enemies for a bunker: `Cache.Groups.Groups` (the infected `GroupsController`) within `weapon.Stats.GetReach(false,0)`.
- ⚠️ **`ResourcesContainer.ForceRemoveResource` CORRUPTS state → next-frame crash** (the game's BugDetector pops
  its window). Use the safe **`TryRemoveResource(id, qty)`** (bool) instead — it respects the game's invariants.
  Guard `GetResourceQuantity` with `HasResource` first (it does `_resources[id]` → KeyNotFound otherwise).
  Global stockpile (top-bar totals, incl. `res_ammo`) = **`StockroomsController.Container`** (`Cache.Stockrooms.Container`),
  NOT `HqController.MainHeadquarter.ResourcesContainer`.
- ⚠️ **BugDetector pops on `LogType.Exception` only** (`Application.logMessageReceived`, `BugDetector.cs:62`) — i.e.
  any **uncaught** exception. `Debug.LogError` (LogType.Error) does NOT trigger it, but ALWAYS try/catch mod code
  in events/UI callbacks so nothing escapes. (Note: BepInEx `LogOutput.log` writer is dead under Proton → use
  `Debug.Log` for diagnostics, lands in Player.log. See [[feedback_ifz_crashlog]].)
- 🧭 **Flare timing on game clock:** `Controllers.Time.TimeController.GameplayTime` / `.IsPaused` — pauses with the
  game + scales with speed. Use it for lifetime/descent so flares freeze on pause (real `Time.time` keeps running).
- 🧭 **Native UI from a mod:** game uses **TMP**. Build a uGUI Canvas in code, grab the font from any live
  `TextMeshProUGUI.font`. `SpriteRenderer` uses the default sprite material (no shader lookup); `TrailRenderer`/
  `LineRenderer` need a material → `Shader.Find("Sprites/Default")`. Gate UI to in-game via `HqController.MainHeadquarter != null`.
- ⚠️ **No new `ResourceID`.** It's a baked 78-value enum + ScriptableObject data + UI — not runtime-moddable. A new
  "ammo type" must be **mod-tracked** (PlayerPrefs) + crafted from an existing resource; native resource-panel rows
  are `ResourceID`+container driven (`SquadStoragePanel.RefreshUI`) so a fake row is fragile — build an own styled panel instead.
