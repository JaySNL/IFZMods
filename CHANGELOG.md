...
- **PerfPack 1.4.0 — late-game / night FPS roughly doubled by LODing hideout patrol spawn points (same horde, no gameplay change).** Measured on a day-193 save: night sat at ~12-13 FPS. Root-caused with the bundled sampler to **one** system — `GroupSpawnPointsController.CustomUpdate` at ~25 ms/frame (≈70% of all game-sim). A hideout-heavy map accumulates tens of thousands of *dormant patrol spawn points* (here **19,259** across 77 hideouts, ~334/hideout — the game's own per-difficulty cap, **not** a leak: the invariant `count ≤ sum(MaxPatrolsCount)` held). These points are the game's lightweight "virtual" form of a patrol (activation is fog-of-war driven), yet the base game **updates every one every frame** — path-move + terrain raycast (`UnitySampleHeight`) + fog detection — *and* leaves a `SphereCollider` live on each even though it's used only for debug-mesh draw / editor gizmos (no triggers; activation doesn't use it). At 19k *moving* colliders that also churned the physics broadphase. PerfPack now: (1) **distance-LODs** the per-frame work — off-screen points tick every Nth frame with `GameplayDeltaTime` scaled ×N so roaming distance and despawn timing stay accurate, near-camera points are untouched; (2) **disables the inert debug colliders**. **Horde size is unchanged** (verified: still 19,259 points). Result: night ~12-13 → **26-28 FPS**, `GameManager.Update` 29 → 15 ms, worst spike 103 → ~50 ms — the bulk of the Unity-side win is the collider disable (~26 ms/frame). Both behaviours are config-gated under `[SpawnPointLod]` and reversible. *(Reportable to the devs — especially the debug colliders left active in a release build.)*
