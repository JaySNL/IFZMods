...
### Added
- **RaiderEscalation 1.0.0 — raider camps that grow, raid, and pay out.** New mod built on the engine's Hideout/Lair system (in IFZ a "Hideout" *is* a "Lair" — one system keyed by `Fraction`; `HideoutsController.HideoutCleared` fires for both bandit camps and swarm nests). Three pillars:
  - **Loot on clear (both fractions).** Wiping any raider camp or swarm nest grants a resource bundle to your stockroom (ammo + scrap + food; fuel at tier 2+, a weapon at tier 3+), scaled by the camp's escalation tier. Hooks `HideoutCleared` and grants via `StockroomsController.Container.ForceAddResource`. Vanilla gives no clear reward (only a mood bump), so this is new for both fractions.
  - **Escalation (raiders only).** Each bandit camp gains a tier every `TierIntervalDays` (default 3, capped at `MaxTier` 4); each tier-up thickens the camp's patrols via `Hideout.TryCreateGroupFromMainGroup`. Swarms are skipped — they already self-escalate (SwarmsIntensity / natural spread).
  - **Raids (raiders only, opt-in pressure).** High-tier camps (`MinTier` 2+) periodically march patrol groups at your nearest HQ via a `MoveOrder` — behaviour that doesn't exist in vanilla (raiders are normally reactive + leashed to their camp). Caveat surfaced in the config: groups stay leashed to their hideout, so distant camps may not reach your base; this dials pressure, not a guaranteed siege.
  - All gentle + fully config-gated (tier interval, max tier, raid min-tier / interval / group count, loot base amount). Depends on IFZModAPI. The "reclaim the fort as a usable building" and "bandit fortifications" ideas remain out of scope — those need engine support (player-faction-tied building system), not a Harmony patch.
