...
### Fixed
- **DarkerNights 1.1.3 — dawn/dusk flicker + dead/fighting night knobs.** (1) **Transition flicker:** during the sunrise/sunset window the game's ColorSwitcher rewrites the grading params every frame; the driver only re-clobbered at 12.5 Hz, so the game's frames bled through between ticks → visible flicker at dawn/dusk. The driver now runs every frame while in the transition window (throttled only at stable times). (2) **`ExposureEV` did nothing live:** night AutoExposure was a tug-of-war between two writers — `NightExposureBias` (a Harmony postfix, default −2.5) and `ExposureEV`/`AbsoluteExposure` (the driver's per-frame ClobberAE, default −7) both wrote the same min/max on alternating frames, so editing either barely moved the result. Removed the redundant `NightExposureBias` config + its `ColorSwitcher.SetNightValue`/`SetRawNightValue` patches; ClobberAE is now the sole AE writer, so `ExposureEV` controls night exposure cleanly and live. (3) **Removed two dead knobs:** `NightAmbientColorMul` and `NightAmbientFactor` were bound but never read (vestiges of the old multiplier approach, superseded by the absolute `NightAbsolute.AmbientIntensity` / `AmbientR/G/B`) — they silently did nothing, now gone. (4) Config edits now apply on the next frame (`Config.SettingChanged` force-apply) instead of waiting for the throttle, and a `Debug.Verbose` toggle logs a live read-back of the grading/exposure params for diagnosing apply issues.
