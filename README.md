...
| **GreenhouseGrow** | ⚠️ *v0.1.0 — experimental.* Build **farms and greenhouses at a chosen size**. During placement, press **,** and **.** to cycle whole size tiers x1 / x2 / x3 / x4 (live-resizing preview). Cost, max staff and daily production all scale with the footprint **area** (tier squared), so a 2x farm costs and yields ~4x. Works on open fields and greenhouses; each farm's size is saved per game. *(Requires 000_IFZModAPI.)* |
