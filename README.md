...
32 mods + ConfigurationManager. All pure-managed `netstandard2.1` — runs on Windows, macOS (Crossover/Wine), Steam Deck / Linux (Proton).
...
| **NoPath - YesPath** | Demolish the **"no path" buildings along the water** that you normally can't remove. Ordering a demolish opens the building's footprint to pathing and injects a reachable work spot onto it, so a real worker walks in and tears it down under normal rules (no instant teardown). Buildings on a land ledge are fixable; ones sitting literally on water are flagged as true isolates (no worker can ever reach those). *(file: `DemolishAnywhere.dll`. Requires 000_IFZModAPI.)* |
| **ExplosivesUnlock** | Unlocks explosives crafting earlier. |
| **Expanded Farming (2x-4x)** | ⚠️ *v0.1.0 — experimental.* Build **farms and greenhouses at a chosen size**. During placement, press **,** and **.** to cycle whole size tiers x1 / x2 / x3 / x4 (live-resizing preview). Cost, max staff and daily production all scale with the footprint **area** (tier squared), so a 2x farm costs and yields ~4x. Works on open fields and greenhouses; each farm's size is saved per game. *(file: `GreenhouseGrow.dll`. Requires 000_IFZModAPI.)* |
...
| **SquadGrenades** | Idle player squads auto-lob grenades and auto-fire mortars at hostile clusters already in range — including from inside buildings and moving vehicles. Friendly-fire safe: cancels if the blast would catch your own squads or any non-hostile group. *(Requires 000_IFZModAPI.)* |
...
| **SwarmFix** | Restores late-game infected night waves that silently stop. Vanilla swarm aggro overflows past the top tier (Infinity/NaN) so no wave ever spawns again while raiders keep attacking; this sanitises the tier lookup so the hordes come back. Standalone. |
| **ThaiLanguage** | ⚠️ *v0.1.1.1 — experimental.* **Thai language support.** The game can already load a Thai translation, but none of its fonts contain Thai glyphs, so Thai text renders as boxes. This registers a bundled **Noto Sans Thai** as a global TextMeshPro fallback, which fixes every label at once, and installs the community Thai translation (4183 keys) into the language dropdown as **ไทย**. The bundled text is pre-segmented with zero-width spaces so Thai wraps instead of overflowing. Optionally uses Thai citizen names and graffiti while playing in Thai. Rough edge: TMP can't do Thai mark positioning, so tone marks over tall consonants may sit wrong. Single self-contained DLL (font + translation embedded). Standalone. |
